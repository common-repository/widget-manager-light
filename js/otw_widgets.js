...
jQuery(document).ready(function() {
	...
	jQuery( '.widget-action' ).on( 'click', function(){
		otw_init_widgets();
	} );
...
function otw_load_visibility_dialog( widget_id, sidebar_id ){

	var req_url = 'admin-ajax.php?action=otw_wml_widget_dialog&sidebar=' + sidebar_id + '&widget=' + widget_id;
	
	jQuery.post( req_url, { 'widget_id': widget_id, 'sidebar_id': sidebar_id}, function(b){
		...
		tb_position = function(){
			...
			jQuery( '#TB_ajaxContent' ).css( 'overflow', 'auto' );
		...
		tb_show( 'Set Visibility', "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
	} );
}
			...
			if( widget_containers.length && sidebar_id != 'wp_inactive_widgets' ){
				...
				widget_containers.each( function(){
					...
					if( widget_id_container.length ){
						...
						var widget_id = widget_id_container.val();
						...
						action_blocks.each( function(){
							...
							{
								var new_action = jQuery( '<input type="button" class="button otw_appearence" name="Set Visibility" value="Set Visibility">' );
								new_action[0].widget_id = widget_id;
								new_action[0].sidebar_id = sidebar_id;
								new_action.on( 'click',  function(){
									
									otw_load_visibility_dialog( this.widget_id, this.sidebar_id );
