...
function otw_init_appearence_dialog(){
	...
	jQuery( '.otw_save_excluded' ).on( 'click',  function(){
	...
	jQuery( 'p.sitem_selected > a, p.sitem_notselected > a').on( 'click',  function(){
		otw_sbm_click_app_item( this );
	});
	
	jQuery('a.all_vis, a.all_invis').on( 'click',  function(){
	...
	jQuery('div.otw_sidebar_wv_filter_show select').on( 'change',  function(){
		var parent_box = jQuery( this ).parents( 'div.sitems');
		otw_sbm_wv_load_items( parent_box );
	});
	jQuery('div.otw_sidebar_wv_filter_order select').on( 'change',  function(){
		var parent_box = jQuery( this ).parents( 'div.sitems');
		otw_sbm_wv_load_items( parent_box );
	});
	jQuery('div.otw_sidebar_wv_filter_clear a').on( 'click',  function( event ){
	...
	jQuery('#otw_dialog_content').find('.sitem_toggle, .sitem_header').on( 'click', function() {
	...
	if( matches = item_node.attr( 'rel' ).match( /^([0-9a-zA-Z\-\_]+)\|([0-9a-zA-Z\-\_]+)\|([0-9a-zA-Z\-\_]+)$/ ) ){
		...
		var settings = {
			...
			success:function( data ){
				...
				jQuery( item_node ).find( 'p.sitem_selected > a, p.sitem_notselected > a').on( 'click',  function(){
					otw_sbm_click_app_item( this );
				});
...
function otw_init_wv_item_pager( item_type ){
	...
	jQuery('#otw_type_' + item_type + '_per_page_field').on( 'change',  function( event ){
		var parent_box = jQuery( this ).parents( 'div.sitems');
		otw_sbm_wv_load_items( parent_box );
	});
	jQuery('#otw_sbm_app_type_' + item_type + ' .otw_sidebar_pager_links a ').on( 'click',  function(){
